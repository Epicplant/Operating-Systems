...
struct cpu {
  ...
  // Depth of pushcli nesting. Invariant: if ncli > 0, then interrupts are
...
// Every process has a `procstate` which describes where in the process
// lifecycle the process is.
enum procstate {
  ...
  SLEEPING,  
  // Ready to be scheduled (procs are only scheduled when state is RUNNABLE).
  RUNNABLE,
  // Currently running.
  RUNNING,
...
struct proc {
  ...
  file_info* infos[NOFILE];           // Array of open files for this process
};
